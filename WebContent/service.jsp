					...
					<div class="container">
						...
						<div class="row">
							<table class="table table-striped"
								...
								<b style="margin: 0%; color: red;">봉사신청서는 파일 양식대로 작성 후, 올려주시면 반영하여 1365봉사센터에 지정하게 됩니다.</b><br><a href="hwp_fileDownload.jsp">파일다운</a>
								<thead>
									<tr>
										<th style="background-color: dark; text-align: center;">번호</th>
										<th style="background-color: dark; text-align: center;">제목</th>
										<th style="background-color: dark; text-align: center;">신청자</th>
										<th style="background-color: dark; text-align: center;">작성일</th>
									</tr>
								</thead>
								<tbody>

									<%
										ExchangeDAO serviceDAO = new ExchangeDAO();
										ArrayList<ExchangeDTO> list = serviceDAO.getList(pageNumber);
										for (int i = 0; i < list.size(); i++) {
									%>

									<tr>
										<td><%=list.get(i).getEx_bbsNo()%></td>
										<td><a
											href="service_view.jsp?ex_bbsNo=<%=list.get(i).getEx_bbsNo()%>"><%=list.get(i).getEx_bbsTitle()%></a></td>
										<td><%=list.get(i).getUserID()%></td>
										<td><%=list.get(i).getEx_bbsDate().substring(0, 11)%></td>
									</tr>
							...
							<%
								...
								if (serviceDAO.nextPage(pageNumber)) {
							%>
							...
							<a href="service_write.jsp" class="btn btn-primary pull-right">글쓰기</a>
